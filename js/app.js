...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!calInput || !weightInput || !modeSelect || !snackSelect || !goBtn || !out) {
    hardFail("A required element is missing in index.html (inputs/select/button). Make sure you’re using the full folder.");
    return;
  }
  ...
  function populateWorkouts() {
    ...
    for (const cat of sortedCats) {
      ...
      // Add a non-clickable header option (works consistently across browsers)
  ...
  function applyWorkoutTheme() {
    ...
    try {
      ...
      const intensity =
        cat === "hiit" || cat === "combat"
          ? "high"
          : cat === "strength" || cat === "sport" || cat === "functional"
            ? "med"
            : "low";
  ...
  goBtn.addEventListener("click", () => {
    ...
    if (brutal) {
      const brutalLine =
        window.BrutalMode?.getBrutalLine?.({
          cal,
          mins,
          minsAdjusted,
      ...
      const contextLine =
        window.BrutalMode?.getBrutalContextLine?.({
          cal,
          mins,
          minsAdjusted,
          burnRate: rate,
